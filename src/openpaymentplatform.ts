...
export class OpenPaymentPlatform {
   ...
   * ```
   */
  initialize(args: { backendUrl: string; merchantKey: string; password: string }): void {
   ...
   * @throws PaymentInitializationException For unexpected/network errors.
   */
  checkStatus(args: { paymentId?: string; orderId?: string }): Promise<CheckStatusResult> {
   ...
   */
  refundPayment(args: { paymentId: string; amount: string }): Promise<string> {
