...
export class AkuratecoHashUtils {
   ...
   */
  static generatePaymentIdHash(args: { id: string; password: string }): string {
   ...
   */
  static generateRefundHash(args: { paymentId: string; amount: string; password: string }): string {
    const toMd5 = `${args.paymentId}${args.amount}${args.password}`;
    return this._generateHash(toMd5);
  }

  private static _generateHash(toMd5: string): string {
